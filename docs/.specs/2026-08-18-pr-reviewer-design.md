...
2. Claude Code ships a built-in `code-review` capability (effort levels low through ultra, correctness + reuse/simplification/efficiency categories, `--comment` to additionally post inline, `--fix` to apply) — a core CLI feature, invoked via `Skill(skill: "code-review", ...)`, not a marketplace plugin. **Name collision warning:** a *separate*, unrelated community marketplace plugin also happens to be named `code-review` (a simpler always-posts bot script with no flags, `gh`-only) — this design depends on the built-in one, never the marketplace plugin of the same name; see the EXIT & DERAILMENT row covering this ambiguity. Without `--comment`, the built-in skill reviews and returns findings only — it does not post anything, which is exactly the behavior `pr-reviewer`'s GitHub path relies on. No GitLab-equivalent skill exists — that half has to be built from `glab` API mechanics, much like maestro's original.
...
| GitHub finding-generation | Delegate directly to `Skill(skill: "code-review", ...)`, no `--comment` (review-only, no posting) | That built-in skill already resolves a GitHub PR target and reviews it — reimplementing it would duplicate what the environment already has, same reasoning maestro itself used for its own built-in `review` Skill. Considered and rejected: self-implementing the GitHub path too, to remove the name-collision risk entirely — rejected because it would throw away `code-review`'s actual review-quality/effort-level machinery for a risk that's mitigated by explicit disambiguation instead (see the agent's EXIT & DERAILMENT table, `docs/.plans/2026-08-18-pr-reviewer.md` Task 4 Step 3). |
...
| Dependency classification | Neither `code-review` nor `/loop` fit cairn's existing hard-required/soft-optional taxonomy (that pattern is for third-party plugins that may or may not be installed). Both are core Claude Code capabilities, assumed present in any session — no install-check, no abort-if-absent logic. | `idea-explorer`'s hard-required pattern and Graphify's soft-optional pattern both exist to handle *plugins* whose presence varies by environment. `code-review` and `/loop` ship with the CLI itself; there's no "not installed" state to detect or degrade from. If a future environment genuinely lacks one, that's an EXIT & DERAILMENT case (unexpected tool failure), not a startup capability check. |
...
Fix-Verification Round run repeatedly via the built-in `/loop` skill — a core Claude Code capability this environment already ships, not a third-party plugin (see the Dependency classification row above) — one pass per tick, terminating on merge/close. `/loop` owns the timer; this agent never sleeps or polls on its own.
