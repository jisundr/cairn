...
- `/cairn-usage` (or `/cairn-usage stop`) — realtime local usage dashboard for the current project, wraps [`codeburn`](https://github.com/getagentseal/codeburn) (`npx codeburn web`). Requires `npx`/Node.js on `PATH` — the only place in cairn with that dependency.
