---
description: "Open a realtime local usage dashboard for this project — token/cost totals, breakdowns by model/version/subagent/skill, a per-session table, a task tracker tab, and a swarms tab tracking Unattended coding-chain tasks. Requires /cairn-setup to have run first."
argument-hint: "[stop]"
---

## Your task

This runs cairn's own dashboard (`scripts/usage_dashboard.py`, stdlib Python, no dependencies) — it reads this project's session transcripts directly from `~/.claude/projects/`, so there's nothing to install or configure first. It does require the project to have run `/cairn-setup`, though — see step 1 below. Its Tracker tab reads `docs/.tasks/TRACKER.md` if present (empty state if not — no dependency on `project-manager` having run). Its Swarms tab reads `docs/.tasks/*/STATE.md` for `Mode: Unattended` tasks, with tmux liveness checking and a pane-tail excerpt when a task is paused at `HANDOFF NEEDED`.
