...
1. Use `Glob` — and only `Glob` — to check for `.claude/skills/impeccable/SKILL.md` in the current project. **Never `Bash ls`/`find`/`test` (or any other shell command) for this check.** `Bash` is reserved solely for Impeccable's own setup scripts (step 3 below), and only after its presence has already been confirmed via this `Glob` call. Do not substitute `Read`, or any other tool, for `Glob` here, and do not assume `Glob` is unavailable without actually attempting the call — if `Glob` genuinely errors, report the exact error rather than silently switching tools.
