...
1. Use `Glob` to check for `.claude/skills/impeccable/SKILL.md` in the current project — **never `Bash ls`/`find`/`test` (or any other shell command) for this check.** `Bash` is reserved solely for Impeccable's own setup scripts (step 3 below), and only after its presence has already been confirmed via this `Glob` call.
