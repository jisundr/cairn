...
**Implementation status:** everything described below is built and shipped. The `cairn-dashboard` repo holds the full Vite + React + TypeScript SPA (`package.json`, `vite.config.ts`, `src/`, plus a committed `dist/` build), and the parent repo's `scripts/usage_dashboard.py` serves `/api/usage`, `/api/tracker`, and `/api/swarms` alongside the SPA's static bundle (`serve_static()`).
...
- The existing Python API surface (`/api/usage`, `/api/tracker`, `--task-report`, `--window-report`) is stable and was not redesigned by this effort, only extended (`/api/swarms`) and rehomed for static-file serving.
