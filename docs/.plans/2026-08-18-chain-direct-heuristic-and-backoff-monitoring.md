...
- Backoff: 1 min start, doubles per unchanged tick, caps at 30 min, full reset to 1 min on any change. `STALLED` after 3 consecutive unchanged ticks once at the 30-min cap — never for a task whose `Phase` is `PUBLISH` or `HANDOFF NEEDED`.
