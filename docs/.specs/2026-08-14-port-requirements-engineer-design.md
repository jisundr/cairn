...
- `graphql-guide` — conditional load when the API surface is GraphQL. **Correction: NOT self-contained as originally stated** — it's a 3-section, multi-audience file: Section 1 ("API Design Standards") is scoped to `solution-architect`; Sections 2 ("Ariadne backend implementation") and 3 ("Frontend Consumption") are scoped to `software-engineer` (writing actual Ariadne/TanStack Query code) — an agent not being ported, and code `solution-architect` must never produce ("MUST NOT generate... application code"). Only Section 1 is ported; Sections 2-3 dropped.
...
- **`skills/solution-architecture-writing/SKILL.md`** — the 3 technical doc types + ADR Mode (numbering, sub-modes, immutability rule) + `db-standards-guide`/`api-standards-guide` content merged in + `graphql-guide` Section 1 ONLY (API Design Standards — Sections 2-3 dropped per above, they're for an un-ported code-writing agent). References `skills/mermaid-diagrams/SKILL.md` for `architecture-spec.md`, `db-schema.md`, and ADRs — not `api-spec.md`.
- **`skills/mermaid-diagrams/SKILL.md`** — ported from `mermaid-diagram-guide`, **not as-is**. Its "Required Diagrams by Document Type" table is stale against maestro's own current doc structure — it references `docs/architecture/01_overview.md`/`03_components.md`/`04_integration.md`/`06_deployment.md`, `docs/api-spec/01_endpoints.md`, `docs/db-schema/01_erd.md` (a numbered multi-file architecture layout) when `solution-architect.md` (read directly) actually produces single-file `architecture-spec.md`/`db-schema.md`/`api-spec.md`. The same stale table also claims `project-definition.md`/`prd.md`/`user-flows.md` need diagrams, contradicting those 3 docs' own current templates (verified individually, no diagram placeholders anywhere except one explicit "do NOT use Mermaid" carve-out). **Resolved: the doc-type-mapping table is dropped entirely** — the current, individually-verified doc-type guides (which already state inline where a diagram is wanted: `ux-spec.md`, `architecture-spec.md`, `db-schema.md`, ADRs) are the source of truth for *where* diagrams go, not this table. What's ported is the path-agnostic remainder: Diagram Type Reference (content category → Mermaid diagram type), Diagram Placement Rules (caption format, figure numbering, skip Metadata/Open Questions, node-count split), Diagram Format Rules (fenced code block tag, node label/ID conventions, direction convention). Loaded conditionally, per doc type, by `product-designer` and `solution-architect` — never by `requirements-engineer` (none of its 4 doc types use diagrams, per their own current templates).
